...
static SALT: Lazy<String> = Lazy::new(|| std::env::var("SALT").expect("salt env not be seted"));
...
mod tests {
    ...
    fn test_hash() {
        ...
        let hashed_password = AuthUtils::hash(password.clone()).unwrap();
        let match_result = AuthUtils::verify_hash(password, hashed_password).unwrap();
        assert!(match_result)
    }
}
